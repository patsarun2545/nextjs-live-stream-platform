...
router.post('/', protect, streamerOnly, async (req, res) => {
  try {
    ...
    const user = await require('../models/User').findById(req.user._id).select('+streamKey');
...
router.patch('/:slug', protect, streamerOnly, async (req, res) => {
  try {
    ...
    const allowed = ['title', 'description', 'category', 'tags', 'thumbnail', 'status'];
    allowed.forEach((f) => { if (req.body[f] !== undefined) video[f] = req.body[f]; });
    if (req.body.status === 'ended' && !video.endedAt) video.endedAt = new Date();
    await video.save();
    res.json({ video });
  } catch (err) {
    res.status(400).json({ message: err.message });
...
router.post('/:id/comments', protect, async (req, res) => {
  try {
    const { text } = req.body;
    if (!text?.trim()) return res.status(400).json({ message: 'กรุณาใส่ข้อความ' });
